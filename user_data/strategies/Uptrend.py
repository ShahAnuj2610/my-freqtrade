...
class SuperBuy(Uptrend):
    ...
    generator = IntParameter(0, 100000000000, default=99295874569, optimize=True,
                             space='buy')  # generate unique matrix of conditions for your dataframe
    operators_used_to_compare_between_columns = IntParameter(0, 3, default=3, optimize=True,
                                                             space='buy')  # number of conditions you will keep to build buy signal
    operators_used_to_with_best_point = IntParameter(0, 3, default=1, optimize=True,
                                                     space='buy')  # number of conditions you will keep to build buy signal
    condition_selector = IntParameter(0, 100, default=50, optimize=True,
                                      space='buy')  # how to select the desired conditions beteween all conditions generated (seed random)
    ...
    def find_best_entry_point(self, dataframe: DataFrame, metadata: dict):
        ...
        workdataframe['higher_high_close_ratio'] = workdataframe['higher_high'] / workdataframe[
            'close_shifted_lookehead']
        ...
        filtered_df["shifted_index"] = filtered_df.index - lookahead_candles + self.top_index_criteria[
            'candles_after_dip_to_buy']
    ...
    def common_points_for_every_best_entry(self, dataframe: DataFrame, metadata: dict, columns: list) -> list:
        ...
        if current_pair not in self.best_buy_point_dict:
            self.best_buy_point_dict[current_pair], self.bad_buy_point_dict[current_pair] = self.find_best_entry_point(
                dataframe, metadata)
        ...
        for column in columns:
            ...
            if not count.empty and not column in ['buy', 'buy_tag']:
                ...
                df_all['part_of_best_points'] = 100 * df_all['best_points'] / (
                        df_all['best_points'] + df_all['bad_points'])
            ...
            if column in ['buy', 'buy_tag']:
                ...
                res.append({
                    ...
                    'ratio_diff': count.iloc[0] / all_best_points.shape[0] - all_points_values_count[count.index[0]] /
                                  all_points.shape[0]
                })
        ...
        print("& ( # protections")
        for item in sorted(res, key=lambda x: x['ratio_diff']):
            if (((100 * item['ratio_for_best']) - (100 * item['ratio_for_all'])) > self.top_index_criteria[
                'select_prameter_if_prop_is_x_percent_higher_in_best_points']) and (100 * item['ratio_for_best']) > \
                    self.top_index_criteria['select_parameter_if_in_more_than_x_percent_of_best_points']:
    ...
    def is_same_dimension_as_price(self, dataframe: DataFrame, column_name: str) -> bool:
        if dataframe['close'].dtype != dataframe[column_name].dtype:
            ...
            return False
        return (dataframe[column_name].max() <= dataframe['high'].max() and dataframe[column_name].min() >= dataframe[
            'low'].min())
    ...
    def generate_superbuy_signal(self, dataframe: DataFrame, metadata: dict) -> list:
        ...
        if len(self.columns_to_compare_to_best_point) == 0 and len(self.columns_to_compare_to_volume) == 0 and len(
                self.columns_to_compare_to_price) == 0:
        ...
        for left_elt in self.columns_to_compare_to_price:
            for right_elt in self.columns_to_compare_to_price:
                ...
                buy_conds.append(
                    "(dataframe['" + left_elt + "'] " + self.operators[
                        int(generator)] + " dataframe['" + right_elt + "'])"
                )
        ...
        for column in self.columns_to_compare_to_best_point:
            ...
            buy_conds_best_point.append(
                "(dataframe['" + column + "'] " + self.operators[int(generator)] + " " + str(
                    self.best_buy_point[column]) + ")"
            )
        ...
        try:
            buy_conds = [eval(buy_cond, globals(), {'dataframe': dataframe, 'best_buy_point': self.best_buy_point}) for
                         buy_cond in buy_conds]
        except:
            return []
        return buy_conds
