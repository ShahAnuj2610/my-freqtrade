...
class BB_RTR(IStrategy):
    ...
    # Buy params toggle
    buy_is_dip_enabled = CategoricalParameter([True, False], default=True, space='buy', optimize=True, load=True)
    buy_is_break_enabled = CategoricalParameter([True, False], default=True, space='buy', optimize=True, load=True)

    is_optimize_pump_1 = True
    buy_pump_1_factor = DecimalParameter(1.0, 1.25, default=1.1, optimize=is_optimize_pump_1)

    is_optimize_pump_2 = True
    buy_pump_2_factor = DecimalParameter(1.0, 1.20, default=1.1, optimize=is_optimize_pump_2)

    ## Sell params
    is_optimize_sell_offset = True
    high_offset_2 = DecimalParameter(0.99, 1.5, default=sell_params['high_offset_2'], space='sell',
                                     optimize=is_optimize_sell_offset)

    is_optimize_sell_u_e_2 = True
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        if (
                ...
                and (((last_candle['ema_200'] - last_candle['close']) / last_candle[
            'close']) < self.sell_u_e_2_ema_close_delta.value)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['vwap_width'] = ((dataframe['vwap_upperband'] - dataframe['vwap_lowerband']) / dataframe[
            'vwap_middleband']) * 100
        ...
        informative['bb_width'] = (
                (informative['bb_upperband2'] - informative['bb_lowerband2']) / informative['bb_middleband2'])
